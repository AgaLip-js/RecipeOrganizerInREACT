  ...
  render() {
    ...
    return (
      <div className="calendar-container">
        <nav>
          <a href={"/planner/" + this.props.week.prevWeek}>
            <span>&larr;</span>
          </a>
          Week of{" "}
          {currentWeek.toLocaleString("default", { month: "short" }) +
            " " +
            (currentWeek.getDate() + 1)}
          <a href={"/planner/" + this.props.week.nextWeek}>
            <span>&rarr;</span>
          </a>
        </nav>
        <div className="week">
          <ul>
            <li className="day-name" id="meal-header">
              <div>
                <FontAwesomeIcon
                  title={this.state.status ? "Add Items" : "Close"}
                  icon={this.state.status ? faPlus : faTimes}
